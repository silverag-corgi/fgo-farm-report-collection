...
def __do_logic_that_generate_tot_sum(
    ...
    try:
        ...
        for col_year_month in col_year_months:
            # 初日(収集年月、今月)の取得
            first_date_of_col_year_month: date = \
                datetime.strptime(col_year_month + '-01', '%Y-%m-%d').date()
    ...
    try:
        ...
        for farm_report_list_file_path in farm_report_list_file_paths:
            if os.path.isfile(farm_report_list_file_path) == False:
                ...
                farm_report_list_temp_df: pd.DataFrame = \
        ...
        if len(farm_report_list_df) > 0 and quest_kind in const_util.QUEST_KINDS:
            ...
            # 周回数の揃え
            farm_report_tot_sum_df[const_util.FARM_REPORT_USR_TOT_SUM_HEADER[6]] = \
                farm_report_tot_sum_df[const_util.FARM_REPORT_USR_TOT_SUM_HEADER[6]].apply(
                    lambda data: pyl.round_half_up(data, 2))
            farm_report_tot_sum_df[const_util.FARM_REPORT_USR_TOT_SUM_HEADER[7]] = \
                farm_report_tot_sum_df[const_util.FARM_REPORT_USR_TOT_SUM_HEADER[7]].apply(
                    lambda data: pyl.round_half_up(data, 2))
            farm_report_tot_sum_df[const_util.FARM_REPORT_USR_TOT_SUM_HEADER[8]] = \
...
def __generate_farm_report_qst_tot_sum_file(
    ...
    try:
        ...
        if len(farm_report_list_df) > 0 and quest_kind in const_util.QUEST_KINDS:
            ...
            # 周回数の揃え
            farm_report_tot_sum_df[const_util.FARM_REPORT_QST_TOT_SUM_HEADER[6]] = \
                farm_report_tot_sum_df[const_util.FARM_REPORT_QST_TOT_SUM_HEADER[6]].apply(
                    lambda data: pyl.round_half_up(data, 2))
            farm_report_tot_sum_df[const_util.FARM_REPORT_QST_TOT_SUM_HEADER[7]] = \
                farm_report_tot_sum_df[const_util.FARM_REPORT_QST_TOT_SUM_HEADER[7]].apply(
                    lambda data: pyl.round_half_up(data, 2))
            farm_report_tot_sum_df[const_util.FARM_REPORT_QST_TOT_SUM_HEADER[8]] = \

...
def do_logic(
    ...
    try:
        # ロガー取得
        lg: Logger = mylib.get_logger(__name__)
        ...
        if should_execute == True:
            ...
            # 周回報告一覧生成開始日付の設定
            list_gen_start_date = __generate_list_gen_start_date(
                farm_report_list_file_path, col_first_date)
            
            # 周回報告一覧生成終了日付の設定
            list_gen_end_date = __generate_list_gen_end_date(
...
def __generate_farm_report_summary(
    ...
    try:
        ...
        # クエスト種別による抽出
        farm_report_summary_data_frame: pd.DataFrame = None
        if quest_kind in const_util.QUEST_KINDS:
            farm_report_summary_data_frame = farm_report_list_data_frame.query(
